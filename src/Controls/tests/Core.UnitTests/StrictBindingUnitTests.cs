...
namespace Microsoft.Maui.Controls.Core.UnitTests
{
	...
	public class StrictBindingUnitTests : BindingBaseUnitTests
	{

		public StrictBindingUnitTests()
		{
		...
		// TODO this doesn't work well - we can't create a binding unless we know what the target is...
		protected override BindingBase CreateBinding(BindingMode mode = BindingMode.Default, string stringFormat = null)
		{
			return StrictBinding<MockViewModel>.Create<string>(
				getter: mvm => mvm.Text,
				setter: (mvm, s) => mvm.Text = s,
				handlers: new[] { new Tuple<Func<MockViewModel, object>, string> (mvm=>mvm, "Text") },
				mode: mode,
		...
		public void InvalidCtor()
		{
			Assert.Throws<ArgumentNullException>(() =>
				new StrictBinding<MockViewModel, string, string>(
					getter: null,
					setter: (mvm, s) => mvm.Text = s,
					convert: null,
					convertBack: null,
			...
			bool isDefault)
		{
			...
			var binding = StrictBinding<ComplexMockViewModel>.Create<string>(
				getter: cmvm => cmvm.Model.Model.Text,
		...
		public void ValueUpdatedWithIndexedPathOnOneWayBinding(bool isDefault)
		{
			...
			var property = BindableProperty.Create("Text", typeof(string), typeof(MockBindable), "default value", propertyDefault);
			var binding = StrictBinding<ComplexMockViewModel>.Create<string>(
				getter: cmvm => cmvm.Model.Model[1],
		...
		public void ValueUpdatedWithSelfPathOnOneWayBinding(bool isDefault)
		{
			...
			var property = BindableProperty.Create("Text", typeof(string), typeof(MockBindable), "default value", propertyDefault);
			var binding = StrictBinding<string>.Create<string>(
				getter: cmvm => cmvm,
		...
		public void ValueUpdatedWithSelfPathOnTwoWayBinding(bool isDefault)
		{
			...
			var property = BindableProperty.Create("Text", typeof(string), typeof(MockBindable), "default value", propertyDefault);
			var binding = StrictBinding<string>.Create<string>(
				cmvm => cmvm, (cmvm, s) => cmvm = s, mode: bindingMode, handlers: null);
		...
		public void ValueConverter()
		{
			...
			var binding = StrictBinding<MockViewModel>.Create<string, int>(
				getter: mvm => mvm.Text,
				setter: (mvm, s) => mvm.Text = s,
				converter: converter,
				handlers: new[] {
		...
		public void ValueConverterParameter()
		{
			...
			var binding = StrictBinding<MockViewModel>.Create<string>(
				getter: mvm => mvm.Text,
				setter: (mvm, s) => mvm.Text = s,
				converter: converter,
		...
		{
			// TODO TSource has to be reference type - TypedBinding doesn't have this limitation
			// - it would work if we store the _source as `object` and not as `TSource`

			var converter = new TestConverter<int, string>();

			var property = BindableProperty.Create("Text", typeof(string), typeof(MockBindable), "0");
			var binding = StrictBinding<object>.Create<int, string>(
				getter: mvm => (int)mvm,
				setter: (mvm, s) => mvm = s,
				converter: converter,
				handlers: null);
		...
		public void MultiplePropertyUpdates()
		{
			...
			var progressBinding = StrictBinding<MultiplePropertyViewModel>.Create<float>(
				getter: vm => vm.Progress,
				handlers: new[] { new Tuple<Func<MultiplePropertyViewModel, object>, string>(vm => vm, "Progress") },
				mode: BindingMode.OneWay);
			var doneBinding = StrictBinding<MultiplePropertyViewModel>.Create<int>(
				getter: vm => vm.Done,
				setter: (vm, d) => vm.Done = d,
				handlers: new[] { new Tuple<Func<MultiplePropertyViewModel, object>, string>(vm => vm, "Done") },
		...
		public void OneWayBindingsDontDoubleApplyOnSourceUpdates()
		{
			...
			var bindable = new MockBindable();
			var binding = StrictBinding<ComplexMockViewModel>.Create<int>(cmvm => cmvm.QueryCount, handlers: null, mode: BindingMode.OneWay);
		...
		public void BindingsShouldNotTriggerOtherBindings()
		{
			...
			var bindable = new MockBindable();
			var qcbinding = StrictBinding<ComplexMockViewModel>.Create<int>(cmvm => cmvm.QueryCount, handlers: null, mode: BindingMode.OneWay);
			var textBinding = StrictBinding<ComplexMockViewModel>.Create<string>(cmvm => cmvm.Text, handlers: null, mode: BindingMode.OneWay);
		...
		public void MostDerviedPropertyOnContextSwitchOfSimilarType()
		{
			...
			bindable.BindingContext = vm;
			var binding = StrictBinding<MockViewModel>.Create<string>(
				getter: static mvm => mvm.Text,
				setter: static (mvm, s) => mvm.Text = s,
				handlers: new[] {
					new Tuple<Func<MockViewModel, object>, string>(mvm=>mvm, "Text")
				});
		...
		public void ChainedPartNull()
		{
			...
			var binding = StrictBinding<ComplexMockViewModel>.Create<string>(
			  getter: static cmvm => cmvm.Model.Text,
			  setter: static (cmvm, s) => cmvm.Model.Text = s,
			  handlers: new[] {
					new Tuple<Func<ComplexMockViewModel, object>, string>(cmvm=>cmvm, "Model"),
		...
		public void SetBindingContextBeforeContextBindingAndInnerBindings()
		{
			...
			var bindingItem1 = StrictBinding<Tuple<string, string>>.Create<string, object>(static s => s.Item1);
			var bindingSelf = StrictBinding<string>.Create<string>(static s => s);
		...
		public void SetBindingContextAndInnerBindingBeforeContextBinding()
		{
			...
			view.BindingContext = new Tuple<string, string>("Foo", "Bar");
			var bindingItem1 = StrictBinding<Tuple<string, string>>.Create<string, object>(s => s.Item1);
			var bindingSelf = StrictBinding<string>.Create<string>(s => s);
			label.SetBinding(Label.TextProperty, bindingSelf);
			label.SetBinding(BindableObject.BindingContextProperty, bindingItem1);
		...
		public void SetBindingContextAfterInnerBindingsAndContextBinding()
		{
			var label = new Label();
			var view = new StackLayout { Children = { label } };
			var bindingItem1 = StrictBinding<Tuple<string, string>>.Create<string>(s => s.Item1);
			var bindingSelf = StrictBinding<object>.Create<object>(s => s);

			label.SetBinding(Label.TextProperty, bindingItem1);
			label.SetBinding(BindableObject.BindingContextProperty, bindingSelf);
			view.BindingContext = new Tuple<string, string>("Foo", "Bar");
		...
		public void Convert()
		{
			...
			slider.SetBinding(Slider.ValueProperty,
				StrictBinding<MockViewModel>.Create<string, double>(
					getter: mvm => mvm.Text,
					setter: (mvm, s) => mvm.Text = s,
		...
		public void FailToConvert()
		{
			...
			slider.SetBinding(Slider.ValueProperty, StrictBinding<ComplexMockViewModel>.Create<ComplexMockViewModel, double>(static mvm => mvm.Model));
		...
		public void NullPropertyUpdatesAllBindings(bool useStringEmpty)
		{
			...
			bindable.BindingContext = vm;
			bindable.SetBinding(MockBindable.TextProperty, StrictBinding<NullViewModel>.Create<string>(nvm => nvm.Foo, handlers: new[] {
				new Tuple<Func<NullViewModel, object>, string>(nvm=>nvm,"Foo")
			}));
			bindable.SetBinding(MockBindable2.Text2Property, StrictBinding<NullViewModel>.Create<string>(nvm => nvm.Bar, handlers: new[] {
		...
		public void BindingSourceOverContext()
		{
			...
			var bindingSelfSource = StrictBinding<string>.Create<string>(s => s, source: "bindingsource");
		...
		public async Task BindingUnsubscribesForDeadTarget()
		{
			...
			create = () =>
			{
				...
				var binding = StrictBinding<TestViewModel>.Create<string>(vm => vm.Foo, (vm, s) => vm.Foo = s, handlers: new[] {
		...
		public void IndexedViewModelPropertyChanged()
		{
			...
			var binding = StrictBinding<Tuple<IndexedViewModel, object>>.Create<object>(
				vm => vm.Item1["Foo"],
				(vm, s) => vm.Item1["Foo"] = s,
				handlers: new[] {
		...
		public void OneTimeBindingDoesntUpdateNeedSettersOrHandlers()
		{
			...
			var b1t = StrictBinding<MockViewModel>.Create<string>(v => v.Text);
		...
		public void SpeedTestApply()
		{
			...
			BindingBase binding = StrictBinding<MockViewModel>.Create<string>(
				getter: mvm => mvm.Text,
				setter: (mvm, s) => mvm.Text = s,
				handlers: new[] {
					new Tuple<Func<MockViewModel, object>, string> (mvm=>mvm, "Text")
			...
			binding = StrictBinding<MockViewModel>.Create<string>(
				getter: mvm => mvm.Text,
				setter: (mvm, s) => mvm.Text = s);
			...
			throw new XunitException($"Applying {it} StrictBindings\t\t\t: {swtb.ElapsedMilliseconds}ms.\nApplying {it} StrictBinding (without INPC)\t: {swtbh.ElapsedMilliseconds}ms.\nApplying {it} Bindings\t\t\t: {swb.ElapsedMilliseconds}ms.\nSetting  {it} values\t\t\t\t: {swsv.ElapsedMilliseconds}ms.");
		}
		...
		public void StrictBindingsShouldNotHang()
		{
			var typedBinding = StrictBinding<VM3650>.Create<string>(
				vm => vm.Title,
				(vm, s) => vm.Title = s,
				handlers: new Tuple<Func<VM3650, object>, string>[] {
